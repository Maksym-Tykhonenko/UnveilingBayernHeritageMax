...
const UnvelingLoadingPage = () => {
  ...
  const letterOpacityValues = useRef(
  ...
  useEffect(() => {
    const unvHerritageLoadingOfUser = async () => {
      try {
        ...
        const isHerritagePresentOnb = await AsyncStorage.getItem(
          'isHerritagePresentOnb',
        );
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      Animated.stagger(
        100,
        letterOpacityValues.map(animValue =>
          Animated.timing(animValue, {
            toValue: 1,
            duration: 50,
            useNativeDriver: true,
...
const unveilingStyles = dimensions =>
  StyleSheet.create({
    images: {
